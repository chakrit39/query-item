...
with col5:
    ...
    Select_type = st.selectbox("ประเภท", List_type[1:-0].sort()  , key='List_type')
    List_place = ['ทั้งหมด']
    for i in df.สถานที่.unique(): List_place.append(i)
    Select_place = st.selectbox("สถานที่", List_place[1:-0].sort() , key='List_place')
    
with col6:
    List_name = ['ทั้งหมด']
    for i in df.รายการ.unique(): List_name.append(i)
    Select_name = st.selectbox("รายการ", List_name[1:-0].sort() , key='List_name')
    
    List_status = ['ทั้งหมด']
    for i in df.สภาพครุภัณฑ์.unique(): List_status.append(i)
    Select_status = st.selectbox("สภาพครุภัณฑ์", List_status[1:-0].sort() , key='List_status')
