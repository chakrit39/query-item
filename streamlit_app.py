...
with col5:
    ...
    Select_type = st.selectbox("ประเภท", List_type, key='List_type', disabled=st.session_state.List_type)
    List_place = ['ทั้งหมด']
    for i in df.สถานที่.unique(): List_place.append(i)
    Select_place = st.selectbox("สถานที่", List_place, key='List_place', disabled=st.session_state.List_place)
    
with col6:
    List_name = ['ทั้งหมด']
    for i in df.รายการ.unique(): List_name.append(i)
    Select_name = st.selectbox("รายการ", List_name, key='List_name', disabled=st.session_state.List_name)
    
    List_status = ['ทั้งหมด']
    for i in df.สภาพครุภัณฑ์.unique(): List_status.append(i)
    Select_status = st.selectbox("สภาพครุภัณฑ์", List_status, key='List_status', disabled=st.session_state.List_status)
